  ...
  return (
    <div className="detail-content program-screen">
      <section className="card proposal-card">
        ...
        <h2>{SPLITS[plan.split]?.label} · 주 {plan.daysPerWeek}회</h2>
        <p>{plan.goalSummary}</p>
        <div className="proposal-facts"><span>회당 {plan.durationMinutes}분</span><span>{plan.focus}</span>{plan.backVariety && <span>등 운동마다 종목 변경</span>}</div>
      ...
      <section className="card proposal-card">
        ...
        <div className="scope-options">
          {["today", "ongoing", "goal"].map((item) => <button key={item} className={scope === item ? "selected" : ""} onClick={() => setScope(item)}>{item === "today" ? "오늘만" : item === "ongoing" ? "앞으로 계속" : "목표 변경"}</button>)}
        </div>
